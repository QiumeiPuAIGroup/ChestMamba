...
from utils.classify_3_dataset import COVIDDataset
...
def get_argparse():
    ...
    # Train Options
    parser.add_argument('--batch-size', type=int, default=16, help='batch size for training (default: 32)')
    parser.add_argument('--workers', type=int, default=2, help='number of data loading workers')
    parser.add_argument('--epochs', type=int, default=300, help='number of epochs for training (default: 20)')
    ...
    parser.add_argument('--name', type=str, default='Mamba_classify_epoch300_3classify_2292',
...
def eval(model_name, use_pretrained, save_dir, test_loader):
    ...
    classification_metrics(Y_test=y_true, Y_pred=y_pred, n=3)
...
if __name__ == '__main__':
    ...
    dataset = COVIDDataset(root='data/chestxray_dataset', transform=transform_, train=True)
    train_size = int(len(dataset) * 0.9)
    val_size = len(dataset) - train_size
    train_dataset, val_dataset = random_split(dataset, [train_size, val_size], generator=torch.Generator().manual_seed(1234))
    test_dataset = COVIDDataset(root='data/chestxray_dataset', transform=transform_, train=False)
    print_info()
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=30, eta_min=0.00001, last_epoch=-1) # 动态更新学习率
